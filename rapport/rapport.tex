...
\caption{Visualisation of the functions for each algorithms.}
\label{fig:line}
\end{figure}
We can see that for \textbf{DGD}, the curve is slightly offset from the others. This may be due to its bias, since a constant step size was used here. There is still an error. If we look at \textbf{GT}, \textbf{ADMM}, and \textbf{DC}, the three curves are virtually indistinguishable.

\bigskip
We can try changing several parameters on the communication graphs to see how they affect the algorithms and their convergence: Directed, Packet loss, Asynchronous. 
...
\begin{figure}[H]
    ...
    \caption{Convergence broken}
    \label{fig:visualisation}
\end{figure}

Because of these modifcations, we can use the Push-sum algorithm to solve the convergence problem. We have tried to implement it on DGD. Here are the results. This method helps DGD to have a lower error and converge faster.  
\begin{figure}[H]
    \centering
...
Finally, we can study the case of a larger \textbf{n}.In order to maintain a valid Nyström approximation, we increase the model dimension $m$ in proportion to $\sqrt{n}$.

 We have implemented a dynamic search procedure to determine the system's maximum capacity. The algorithm iteratively increases $n$ and measures two key metrics:
\begin{itemize}
...
Examining the generated figures (speed vs. $n$) allows us to draw several conclusions:
\begin{enumerate}
...
\end{minipage}
\caption{Speed and optimality ga}
\label{fig:graph_compare}
\end{figure}
...
\end{minipage}
\caption{Functions visualisation}
\label{fig:graph_compare}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{The maximum found is \textbf{8192}}
    \label{fig:visualisation}
\end{figure}
...
To solve the regression problem in a distributed manner while ensuring local data privacy, we implement the DGD-DP algorithm. Following the framework of \textbf{Theorem 2} from \cite{arXiv:2202.01113}, the private update rule is defined as follows:
\[
...
\begin{figure}[H]
\centering
\includegraphics[width=.8\linewidth]{part3_dgd_dp_epsilons (1).pdf}
