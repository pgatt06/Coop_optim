...
At the largest sizes, recomputing every Part I plot against the exact centralized optimum is no longer practical. In that regime, as allowed by the project statement, we rely on surrogate metrics such as the consensus gap and the aggregate gradient norm, and we retune DGD and GT directly on the largest feasible instance.
...
\nu = \frac{C}{\epsilon}
\]
In this implementation, rather than using explicit clipping, we assume the local gradient sensitivity is naturally bounded by $C = 0.01$. This choice of $\nu$ ensures that the privacy budget is respected at each exchange, protecting the local model parameters from being reconstructed by neighboring agents.
